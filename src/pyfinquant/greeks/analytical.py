...
class AnalyticalGreeks:
    ...
    def delta(option: 'Option') -> float:
        ...
        try:
            ...
            d1, _ = BlackScholes._calculate_d1_d2(option)
             ...
             if sigma_sqrt_T == 0: d1 = np.inf if S > K else -np.inf
             else: d1 = (np.log(S / K) + (r - q + 0.5 * sigma**2) * T) / sigma_sqrt_T

        q = option.dividend_yield
        T = option.time_to_maturity

        if T == 0: return 1.0 if option.is_call() and option.underlying_price > option.strike_price else (-1.0 if option.is_put() and option.underlying_price < option.strike_price else 0.0)
        if option.volatility == 0: return np.exp(-q*T) if option.is_call() else -np.exp(-q*T) # Based on deterministic forward price sensitivity
    ...
    def vega(option: 'Option') -> float:
        ...
        if T == 0 or option.volatility == 0: return 0.0
    ...
    def theta(option: 'Option') -> float:
        ...
        try:
            ...
            d1, d2 = BlackScholes._calculate_d1_d2(option)
             ...
             sigma_sqrt_T = sigma * np.sqrt(T)
             if sigma_sqrt_T == 0:
                 d1 = np.inf if S > K else -np.inf; d2 = d1
             else:
        ...
        if option.is_call():
            ...
            theta = term1 - r * K * np.exp(-r * T) * cdf_d2 \
                        + q * S * np.exp(-q * T) * cdf_d1
        elif option.is_put():

            theta = term1 + r * K * np.exp(-r * T) * cdf_neg_d2 \
