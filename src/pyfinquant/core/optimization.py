...
def minimize_risk(returns: Union[np.ndarray, pd.DataFrame], 
    ...
    if target_return is not None:
        constraints.append({'type': 'eq', 'fun': lambda x: np.sum(x * returns.mean()) - target_return})
    
    initial_weights = np.ones(n_assets) / n_assets
    result = minimize(objective, initial_weights, method='SLSQP', bounds=bounds, constraints=constraints)
    return result.x

def maximize_sharpe(returns: Union[np.ndarray, pd.DataFrame],
...
def maximize_sortino(returns: Union[np.ndarray, pd.DataFrame],
                    risk_free_rate: float = 0.0,
                    bounds: Optional[List[tuple]] = None) -> np.ndarray:
    """
...
def minimize_tracking_error(returns: Union[np.ndarray, pd.DataFrame],
